...
-(int) startSimulation {
    //No stopping and starting for now
    if(started) {
        return 1;
    }
    ...
    for(int gen_count = 0; gen_count < generationCount; gen_count++) {
            ...
            for(col_count = 0; col_count < colonyCount; col_count++) {
                ...
                for(int i = 0; i < antCount; i++) {
                    ...
                    if(i < n_active_ants) {
                        ...
                        [[ants objectAtIndex:i] setAntStatus:0];
                    }
                }
        ...
        if(colonyCount>1) {
            ...
            for(int i = 0; i < colonyCount; i++) {
                ...
                if([[colonies objectAtIndex:candidate1] seedsCollected] > [[colonies objectAtIndex:candidate2] seedsCollected]) {
                    p1 = candidate1;
                }
                else {
                    p1 = candidate2;
                ...
                if([[colonies objectAtIndex:candidate1] seedsCollected] >
                   [[colonies objectAtIndex:candidate2] seedsCollected]) {
                ...
                if(arc4random() % 100 < crossover_rate) {
                    new_colonies[i].decayRate = [parent1 decayRate];
                }
                else {
                    new_colonies[i].decayRate = [parent2 decayRate];
                }
                if(arc4random() % 100 < crossover_rate) {
                    new_colonies[i].walkDropRate = [parent1 walkDropRate];
                }
                else {
                    new_colonies[i].walkDropRate =[parent2 walkDropRate];
                }
                if(arc4random() % 100 < crossover_rate) {
                    new_colonies[i].searchGiveupRate = [parent1 searchGiveupRate];
                }
                else {
                    new_colonies[i].searchGiveupRate =[parent2 searchGiveupRate];
                }
                if(arc4random() % 100 < crossover_rate) {
                    new_colonies[i].trailDropRate = [parent1 trailDropRate];
                }
                else {
                    new_colonies[i].trailDropRate = [parent2 trailDropRate];
                }
                if(arc4random() % 100 < crossover_rate) {
                    new_colonies[i].dirDevConst = [parent1 dirDevConst];
                }
                else {
                    new_colonies[i].dirDevConst = [parent2 dirDevConst];
                }
                if(arc4random() % 100 < crossover_rate) {
                    new_colonies[i].dirDevCoeff = [parent1 dirDevCoeff];
                }
                else {
                    new_colonies[i].dirDevCoeff = [parent2 dirDevCoeff];
                }
                if(arc4random() % 100 < crossover_rate) {
                    new_colonies[i].dirTimePow = [parent1 dirTimePow];
                }
                else {
                    new_colonies[i].dirTimePow = [parent2 dirTimePow];
                }
                if(arc4random() % 100 < crossover_rate) {
                    new_colonies[i].densityThreshold = [parent1 densityThreshold];
                }
                else {
                    new_colonies[i].densityThreshold = [parent2 densityThreshold];
                }
                if(arc4random() % 100 < crossover_rate) {
                    new_colonies[i].densityConstant = [parent1 densityConstant];
                }
                else {
                    new_colonies[i].densityConstant = [parent2 densityConstant];
                }
                if(arc4random() % 100 < crossover_rate) {
                    new_colonies[i].densityPatchConstant = [parent1 densityPatchConstant];
                }
                else {
                    new_colonies[i].densityPatchConstant =[parent2 densityPatchConstant];
                }
                if(arc4random() % 100 < crossover_rate) {
                    new_colonies[i].densityPatchThreshold = [parent1 densityPatchThreshold];
                }
                else {
                    new_colonies[i].densityPatchThreshold = [parent2 densityPatchThreshold];
                }
                if(arc4random() % 100 < crossover_rate) {
                    new_colonies[i].densityInfluenceConstant = [parent1 densityInfluenceConstant];
                }
                else {
                    new_colonies[i].densityInfluenceConstant = [parent2 densityInfluenceConstant];
                }
                if(arc4random() % 100 < crossover_rate) {
                    new_colonies[i].densityInfluenceThreshold = [parent1 densityInfluenceThreshold];
                }
                else {
                    new_colonies[i].densityInfluenceThreshold = [parent2 densityInfluenceThreshold];
                }
                if(arc4random() % 100 < crossover_rate) {
                    new_colonies[i].activeProportion = [parent1 activeProportion];
                }
                else {
                    new_colonies[i].activeProportion = [parent2 activeProportion];
                }
                if(arc4random() % 100 < crossover_rate) {
                    new_colonies[i].activationSensitivity = [parent1 activationSensitivity];
                }
                else {
                    new_colonies[i].activationSensitivity = [parent2 activationSensitivity];
                }
                if(arc4random() % 100 < crossover_rate) {
                    new_colonies[i].decayRateReturn = [parent1 decayRateReturn];
                }
                else {
                    new_colonies[i].decayRateReturn = [parent2 decayRateReturn];
                }
                
                //Random mutations
                if(arc4random() % 10 == 0) {
                    new_colonies[i].decayRate += randomNormal(0,new_colonies[i].decayRate * 0.05);
                ...
                if(arc4random() % 10 == 0) {
                    new_colonies[i].walkDropRate += randomNormal(0,new_colonies[i].walkDropRate * 0.05);
                ...
                if(arc4random() % 10 == 0) {
                    new_colonies[i].searchGiveupRate += randomNormal(0,new_colonies[i].searchGiveupRate * 0.05);
                ...
                if(arc4random() % 10 == 0) {
                    new_colonies[i].trailDropRate += randomNormal(0,new_colonies[i].trailDropRate * 0.05);
                ...
                if(arc4random() % 10 == 0) {
                    new_colonies[i].dirDevConst += randomNormal(0,0.001 + fabs (new_colonies[i].dirDevConst * 0.05));
                    if(new_colonies[i].dirDevConst < 0.0f) {
                        new_colonies[i].dirDevConst = 0;
                    }
                }
                if(arc4random() % 10 == 0) {
                    new_colonies[i].dirDevCoeff += randomNormal(0,0.001 + fabs (new_colonies[i].dirDevCoeff * 0.05));
                    if(new_colonies[i].dirDevCoeff < 0.0f) {
                        new_colonies[i].dirDevCoeff = 0;
                    }
                }
                if(arc4random() % 10 == 0) {
                    new_colonies[i].dirTimePow += randomNormal(0,0.001 + fabs (new_colonies[i].dirTimePow * 0.05));
                    if(new_colonies[i].dirTimePow < 0.0f) {
                        new_colonies[i].dirTimePow = 0;
                    }
                }
                if(usesPheromones) {
                    if(arc4random() % 10 == 0) {
                        new_colonies[i].densityThreshold += randomNormal(0, 0.001 + fabs (new_colonies[i].densityThreshold * .05));
                    }
                    if(arc4random() % 10 == 0) {
                        new_colonies[i].densityConstant += randomNormal(0,0.001 + fabs (new_colonies[i].densityConstant * .05));
                    }
                }
                if(usesSiteFidelity) {
                    if(arc4random() % 10 == 0) {
                        new_colonies[i].densityPatchConstant += randomNormal(0,0.001 + fabs (new_colonies[i].densityPatchConstant * .05));
                    }
                    if(arc4random() % 10 == 0) {
                        new_colonies[i].densityPatchThreshold += randomNormal(0,0.001 + fabs (new_colonies[i].densityPatchThreshold * .05));
                    }
                }
                if(arc4random() % 10 == 0) {
                    new_colonies[i].densityInfluenceConstant += randomNormal(0,0.001 + fabs (new_colonies[i].densityInfluenceConstant * .05));
                }
                if(arc4random() % 10 == 0) {
                    new_colonies[i].densityInfluenceThreshold += randomNormal(0,0.001 + fabs (new_colonies[i].densityInfluenceThreshold * .05));
                }
                if(arc4random() % 10 == 0) {
                    new_colonies[i].activationSensitivity += randomNormal(0,0.001 + fabs (new_colonies[i].activationSensitivity * .05));
                    if(new_colonies[i].activationSensitivity < 0.0f) {
                        new_colonies[i].activationSensitivity = 0.0;
                    }
                }
                if(arc4random() % 10 == 0) {
                    new_colonies[i].decayRateReturn += randomNormal(0,0.001 + fabs (new_colonies[i].decayRateReturn * .05));
            ...
            for(int i = 0; i < colonyCount; i++) {
                ...
                dirDevCoeffSum += [c dirDevCoeff];
                dirTimePowSum += [c dirTimePow];
                ...
                [c setDirDevCoeff:new_colonies[i].dirDevCoeff];
                [c setDirTimePow:new_colonies[i].dirTimePow];
            ...
            bestColony.dirDevCoeff = dirDevCoeffSum/colonyCount;
            bestColony.dirTimePow = dirTimePowSum/colonyCount;
...
-(void) run {
    ...
    for(int ant_count = 0; ant_count < antCount; ant_count++) {
        ...
        //Update pheromones
        if([ant penDown] == true) {
            grid[[ant x]][[ant y]].p2 = grid[[ant x]][[ant y]].p2 + deposit_rate_p2;
            grid[[ant x]][[ant y]].p2_time_updated = updateCount;
        }
        
        //Searching ants pick up food
        if([ant rfidX] >= 0 && [ant rfidX] < grid_width && [ant rfidY] >= 0 && [ant rfidY] < grid_height) {
            if(([ant antStatus] == 3) && grid[[ant rfidX]][[ant rfidY]].food > 0) {
                ...
                for(int k = -smell_range; k <= smell_range; k++) {
                    if([ant x] + k < 0 || [ant x] + k >= grid_width) {
                        continue;
                    }
                    
                    for(int l = -smell_range; l <= smell_range; l++) {
                        if([ant y] + l < 0 || [ant y] + l >= grid_height) {
                            continue;
                        }
                        if(grid[[ant x] + k][[ant y] + l].food > 0) {
                            density_count++;
                        }
                    }
                }
                ...
                if(arc4random() % 100 / 100.0f <= (density_count / [[colonies objectAtIndex:col_count] densityThreshold] + [[colonies objectAtIndex:col_count] densityConstant])) {
                    //Lay a trail
                    [ant setPenDown:YES];
                    grid[[ant x]][[ant y]].p1 += 20;
                }
                
                if(arc4random() % 100 / 100.0f >= (density_count / [[colonies objectAtIndex:col_count] densityInfluenceThreshold] + [[colonies objectAtIndex:col_count] densityInfluenceConstant])) {
                    //Will follow pheromone trails from the nest if any exist
                    [ant setInfluenceable:YES];
                }
                
                if(arc4random() % 100 / 100.0f <= (density_count / [[colonies objectAtIndex:col_count] densityPatchThreshold] + [[colonies objectAtIndex:col_count] densityPatchConstant])){
        ...
        //Arrive at nest
        if([ant antStatus] == 2 && grid[[ant x]][[ant y]].nest == true) {
            [ant setPrevX:-1];
            [ant setPrevY:-1];
            ...
            if([ant carrying] > 0) {
                ...
                for(int k = -1; k <= 1; k++) {
                    ...
                    for(int l = -1; l <= 1; l++) {
                        ...
                        if(sqrt(pow ([ant x] - nestx, 2) + pow ([ant y] - nesty, 2)) - sqrt (pow (i_k - nestx,2) + pow (j_l - nesty,2)) <= 0) {
              
                            //Sum pheromones within 1 square of nest -- adjust pheromone strength as the square of distance from current location
                            sum_pheromone += grid[i_k][j_l].p2 / pow (sqrt (pow ([ant x] - i_k, 2) +pow ([ant y] - j_l, 2)), 2);
                        }
                    }
                }
                ...
                if([ant penDown] == true) {
                    ...
                    for(int n = 0; n < antCount; n++) {
                        ...
                        if(([ant2 antStatus] == 0) && (arc4random() % (1000) / (1000.0f) < (return_pheromone * [[colonies objectAtIndex:col_count] activationSensitivity])/(antCount))) {
            ...
            else if(([ant returnX] != -1) & ([ant returnY] != -1)) {
        ...
        if([ant antStatus] == 1) {
                ...
                for(int l = -1; l <= 1; l++) {
                    ...
                    if(sqrt(pow ([ant x] - nestx, 2) + pow ([ant y] - nesty, 2)) - sqrt (pow (i_k - nestx, 2) + pow (j_l - nesty, 2)) <= 0) {
                        ...
                        if(grid[i_k][j_l].p2 > 0) {
                            ...
                            grid[i_k][j_l].p2 *= pow ((1 - [[colonies objectAtIndex:col_count] decayRate]),(updateCount - grid[i_k][j_l].p2_time_updated));
                        ...
                        if(grid[i_k][j_l].p2 > most_pheromone && grid[i_k][j_l].p2 > 0.0f) {
                        ...
                        back_pheromone += grid[i_k][j_l].p2 / pow (sqrt(pow ([ant x] - i_k, 2) + pow ([ant y] - j_l, 2)), 2);
                    }
                }
            }
            ...
            if((arc4random() % 1000 / 1000.0f > sum_pheromone / back_pheromone) & (sqrt (pow ([ant x] - nestx, 2) + pow ([ant y] - nesty, 2)) > sqrt (2 * pow (smell_range,2))) & (grid[[ant x]][[ant y]].p1 > 0.0)) {
            ...
            //ants have a small probability of dropping off the trail each time step
            else if(arc4random() % 10000 / 10000.0f < [[colonies objectAtIndex:col_count] trailDropRate]){
                ...
                //Accept a move with probability proportional to the ratio of the distance of the square to
                //be moved to and the adjacent square with the greatest decrease in distance from the patch
                int new_x = -1, new_y = -1;
                while(!move_accepted) {
                    ...
                    if(sqrt(pow ([ant x] - nestx, 2) + pow ([ant y] - nesty,2)) - sqrt (pow (new_x - nestx,2) + pow (new_y - nesty, 2)) > 0) {
                        continue;
                    }
          
                    if((most_pheromone <= 0.0f) || (arc4random() % 100 / 100.0f < grid[new_x][new_y].p2 / most_pheromone)) {
        ...
        if([ant antStatus] == 5) {
            if(([ant returnX] == -1 && [ant returnY] == -1) ||
               ([ant x] == [ant returnX] && [ant y] == [ant returnY])) {
                ...
                if([ant x] + newdx >= 0 && [ant x] + newdx < grid_width && [ant y] + newdy >= 0 && [ant y] + newdy < grid_height) {
                ...
                for(int k = -1; k < 2; k++) {
                    ...
                    for(int l = -1; l < 2; l++) {
                        ...
                        //Distance
                        if(sqrt(pow([ant x] - [ant returnX],2) + pow ([ant y] - [ant returnY],2)) - sqrt (pow (i_k -[ant returnX],2) + pow (j_l - [ant returnY],2)) > most_distance) {
              
                            most_distance = sqrt (pow([ant x] - [ant returnX],2) + pow ([ant y] - [ant returnY],2)) - sqrt(pow (i_k -[ant returnX],2) + pow(j_l-[ant returnY],2));
                        }
                    }
                }
                ...
                while(!move_accepted) {
                    ...
                    if((most_distance <= 0.0f) || arc4random() % 100 / 100.0f < (sqrt(pow ([ant x] - [ant returnX], 2) + pow ([ant y] - [ant returnY],2)) - sqrt (pow (new_x - [ant returnX],2) + pow (new_y - [ant returnY],2))) / most_distance) {
        ...
        if([ant antStatus] == 4) {
            if((arc4random() % 10000 / 10000.0f < [[colonies objectAtIndex:col_count] walkDropRate])) {
            ...
            //Optimal X and Y move given the ant's chosen direction. Move may not be possible (i.e., not on
            //the grid) but the ant will try to get as close as possible
            float idealx;
            float idealy;
            
            if(fabs (sin ([ant direction] * M_PI / 180)) > fabs (cos ([ant direction] * M_PI / 180))) {
                idealx = [ant x] + 50 * (cos ([ant direction] * M_PI / 180) / fabs (sin ([ant direction] * M_PI / 180)));
                idealy = [ant y] + 50 * (sin ([ant direction] * M_PI / 180) / fabs (sin ([ant direction] * M_PI / 180)));
            }
            else {
                idealx = [ant x] + 50 * (cos ([ant direction] * M_PI / 180) / fabs (cos ([ant direction] * M_PI / 180)));
                idealy = [ant y] + 50 * (sin ([ant direction] * M_PI / 180) / fabs (cos ([ant direction] * M_PI / 180)));
            }
            ...
            for(int k = -1; k < 2; k++) {
                ...
                for(int l = -1; l < 2; l++) {
                    ...
                    //Distance
                    if(sqrt (pow ([ant x] - idealx, 2) + pow ([ant y] - idealy,2)) - sqrt (pow (i_k - idealx, 2) + pow (j_l - idealy,2)) > most_distance) {
            
                        most_distance = sqrt (pow ([ant x] - idealx, 2) + pow ([ant y] - idealy, 2)) - sqrt (pow (i_k - idealx, 2) + pow (j_l - idealy,2));
            ...
            while(!move_accepted) {
                ...
                if((most_distance <= 0.0f) || arc4random() % 100 / 100.0f < (sqrt (pow ([ant x] - idealx, 2) + pow ([ant y] - idealy, 2)) - sqrt (pow (new_x - idealx,2) + pow (new_y - idealy,2))) / most_distance) {
        ...
        if([ant antStatus] == 3) {
            if([ant sinceMove] < search_delay) {
                ...
                while(!move_accepted && [ant antStatus] == 3) {
                    ...
                    if((([ant searchDirection] == 360.0)) || ([ant x] == 0) || ([ant y] == 0) || ([ant x] == grid_width - 1) || ([ant y] == grid_height - 1)) {
                        [ant setSearchDirection:(arc4random() % 360)];
                    }
                    ...
                    if([ant searchTime] >= 0.0) {
                        d_theta = randomNormal(0,(([[colonies objectAtIndex:col_count] dirDevCoeff] / pow ([ant searchTime],[[colonies objectAtIndex:col_count]dirTimePow])) +[[colonies objectAtIndex:col_count] dirDevConst]));
                    }
                    else {
                        d_theta = randomNormal(0,[[colonies objectAtIndex:col_count] dirDevConst]);
                    }
          
                    //ants pick a new direction only every 30 cm, like the antbots
                    if(updateCount % 3 == 0) {
                        new_direction = pmod([ant searchDirection] + (d_theta * 180.f / M_PI), 360);
                    ...
                    //Ants that do not smell food probabilistically give up searching and return to nest
                    if(arc4random() % 10000 / 10000.0f < [[colonies objectAtIndex:col_count] searchGiveupRate]) {
                ...
                if([ant antStatus] == 3) {
                    //ant turns in the direction it intends to travel, QR reader rotates in front of it
                    ...
                    if([ant x] + newdx >= 0 && [ant x] + newdx < grid_width && [ant y] + newdy >= 0 && [ant y] + newdy < grid_height) {
                        ...
                        [ant setRfidX:[ant x] + newdx];	//place QR reader in front of ant
                        [ant setRfidY:[ant y] + newdy];
            
                        if(grid[[ant rfidX]][[ant rfidY]].food > 0) {
                            found_a_seed = true;
                        }
                    }
          
                    //if the QR reader has not passed over a seed then move the ant to its new location.
        ...
        //Return to the nest
            ...
            if(([ant carrying] != 0) && ([ant sinceMove] < return_delay)) {
                [ant setSinceMove:[ant sinceMove]+1];
            }
            else {
                float most_distance = 0.0f;
                ...
                for(int k = -1; k < 2; k++) {
                    ...
                    for(int l = -1; l < 2; l++) {
                        ...
                        //Distance
                        if(sqrt(pow ([ant x] - nestx, 2) + pow ([ant y] - nesty,2)) - sqrt (pow (i_k - nestx,2) + pow (j_l - nesty,2)) > most_distance) {
              
                            most_distance = sqrt (pow ([ant x] - nestx, 2) + pow ([ant y] - nesty,2)) - sqrt (pow (i_k - nestx,2) + pow (j_l - nesty,2));
                        }
                    }
                }
                ...
                while(!move_accepted) {
                    ...
                    if((most_distance <= 0.0f) || arc4random() % 100 / 100.0f < (sqrt(pow([ant x] - nestx, 2) + pow ([ant y] - nesty,2)) - sqrt (pow (new_x - nestx,2) + pow (new_y - nesty,2))) / most_distance) {
...
-(void) initDistribution {
    ...
    n_food_orange += numberOfSeeds * distributionClustered;
...
-(void) layoutTags {
    ...
    for(int clump_count = 1; clump_count <= num_each_clump; clump_count++) {
        ...
        while(food_count < n_food_red) {
            ...
            int randx = arc4random() % (tempradius * 2) + clumpx - tempradius;
            int randy = arc4random() % (tempradius * 2) + clumpy - tempradius;
